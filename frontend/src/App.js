...
function PuneJnTab({ schedule, loading, error, newTrain, setNewTrain, handleAddTrain, handleDelete }) {
  ...
  useEffect(() => {
    schedule.forEach(row => {
      ...
      fetch(`${process.env.REACT_APP_API_URL}/api/live-status/${row.train_no}`)
        .then(r => r.json())
...
function App({ loggedInUser, onLogout }) {
  ...
  const fetchSchedule = () => {
    fetch(`${process.env.REACT_APP_API_URL}/api/schedule`)
  ...
  const fetchNearby = () => {
    fetch(`${process.env.REACT_APP_API_URL}/api/nearby-stations`)
      .then(r => r.json())
  ...
  const handleAddTrain = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/add-train`, {
      method: 'POST',
  ...
  const handleDelete = (trainNo) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/api/delete-train/${trainNo}`, { method: 'DELETE' })
